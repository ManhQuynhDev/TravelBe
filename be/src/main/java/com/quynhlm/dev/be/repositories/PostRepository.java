...
//     )
//     SELECT * FROM (
//         -- Top 10 bài viết có reaction_count cao nhất (loại trừ các bài viết trong top_posts)
//         (
//             SELECT
//                 p.user_id AS owner_id,
//                 p.id AS post_id,
...
//             WHERE
//                 p.status = 'PUBLIC'
//                 AND p.id NOT IN (SELECT post_id FROM top_posts) -- Loại bỏ các bài viết trong top 10
//             GROUP BY
//                 p.id, p.user_id, p.content, p.location_id, p.hastag, p.status, m.type, p.create_time, a.fullname, a.avatar_url
//             ORDER BY
//                 COUNT(DISTINCT r.id) DESC
